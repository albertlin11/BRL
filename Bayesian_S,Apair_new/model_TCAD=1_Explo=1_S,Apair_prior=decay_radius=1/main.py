...
os.makedirs("data_old_reward_radius=1_2", exist_ok = 1)
...
def change_TCAD(TCAD,reward,action,list):
    if action not in list:
        mul = abs(reward / TCAD[action])
        TCAD[action] = reward
        return TCAD, mul
    else:
        mul = 1
...
for k in range(6):
    ...
    filepath2 = "./data_old_reward_radius=1_2/RL_network="+str(network_num)+ "_batch_size="+\
        str(batch_size_num)+".csv"
    ...
    input_list = np.array([[1,1],[1025,1025]])
    ...
    for ep in range(5):
        ...
        while not done:
            ...
            if np.random.randint(1,11)/10 <= epsilon:
                action = np.random.randint(0,1024)
            else:
                ...
                if select == 1:
                    ...
                    pred = np.reshape(pred,(1,-1))[0]
                    pred = -(10 ** (-pred))
                pred = pred + 2*prior / divide
                action = np.argmax(pred)


            next_state,reward,done,env_timestep,Rs,true_label = environment.response(action)
            ...
            memory = [state_index,action+1,next_state_index,done]
            ...
            for j in range(len(save_state)):
                if save_state[j][3] == 1:
                    ...
                    pred1 = -(10 ** (-pred1))
                    pred2 = -(10 ** (-pred2))
                    pred1 = pred1 + 2*prior/divide
                    pred2 = pred2 + 2*prior/divide
            ...
            prior, mul= change_TCAD(prior,reward,action,save)
            if radius == 1:
