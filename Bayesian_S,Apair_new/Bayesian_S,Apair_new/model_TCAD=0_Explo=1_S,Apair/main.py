...
os.makedirs("data_old_reward_prior=0_2", exist_ok = 1)
...
for k in range(3,6):
    ...
    filepath2 = "./data_old_reward_prior=0_2/RL_network="+str(network_num)+ "_batch_size="+\
        ...
        while not done:
            ...
            for j in range(len(save_state)):
                if save_state[j][3] == 1:
                    ...
                    pred1 = -(10 ** (-pred1))
                    pred1 = pred1
                    action = np.argmax(pred1)
                    save_q1 = -np.log10(save_reward[j]+0.2*np.abs(pred1[action]))
                    if save_q1 <= -3:
                        save_q1 = -3
